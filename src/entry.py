...
class Default(WorkerEntrypoint):
    ...
    async def _handle_webhook_notification(self, request):
        ...
        try:
            ...
            console.log(f"Webhook received: {len(notifications)} notifications")
            ...
            for i in range(len(notifications)):
                ...
                if change_type != "created":
                    console.log(f"Skipping non-created notification: {change_type}")
                    continue
                ...
                if message_id:
                    ...
                    console.warn(f"Could not extract message ID from resource: {resource}")
            ...
            return Response("", status=202)  # Still return 202 to prevent retries
    ...
    async def _process_email(self, message_id: str):
        ...
        try:
            ...
            try:
                ...
                console.log(f"Applying category '{category_name}' to email {message_id[:20]}...")
                success = await apply_category_to_email(access_token, user_email, message_id, category_name)
                if success:
                    console.log(f"Category '{category_name}' applied successfully")
                else:
                    console.warn(f"Failed to apply category '{category_name}' - check Mail.ReadWrite permission")
            except Exception as cat_err:
                console.error(f"Error applying category '{category_name}': {cat_err}")
            ...
            for sub in subscriptions:
                ...
                if sub_id:
                    try:
                        ...
                        console.log(f"Renewed subscription {sub_id[:20]}... - expires {result.get('expiration', 'unknown')}")
                        renewed_count += 1
                    except Exception as e:
                        console.error(f"Failed to renew subscription {sub_id[:20]}...: {e}")
            
            console.log(f"Subscription renewal complete: {renewed_count}/{len(subscriptions)} renewed")
